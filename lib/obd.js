...
OBDReader.prototype.connect = function () {
    ...
    btSerial.connect(this.address, this.channel, function () {
        ...
        //Write an ATS0, this disables spaces in in output, which is faster!
        self.write('ATS0');
    ...
    this.btSerial = btSerial; //Save the connection in OBDReader object. TODO: Redo this.

//Old code, TODO: review soon.
//    queue.setMinTime(1000); //Set minimal writing delay. (sleep replacement)
//    queue.on('write',
//        function (jinn, data) {
...
//    queue.execute();
    ...
    }, 200); //Could be slightly lower 190?, but 200 is safe. This is for candy-stick OBDSIM.
...
OBDReader.prototype.write = function (message) {
    ...
    //queue.pushTask('write', message + '\r');
    //queue.execute();
    if(queue.length < 256) {
 ...
 * Starts polling.
 * @this {OBDReader}
 * @param {number} interval Frequency how often all variables should be polled. (in ms) Default = 1000 ms.
 */
